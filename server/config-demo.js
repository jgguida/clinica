var config = {
  ...
  couchAdminUser: "aline",
  couchAdminPassword: "Aline1*",
...
config.couchDbURL =
  config.getProtocol(config.couchDbUseSsl) +
  config.couchDbServer +
  ":" +
  config.couchDbPort;
config.couchAuthDbURL =
  config.getProtocol(config.couchDbUseSsl) +
  config.couchCredentials() +
  config.couchDbServer +
  ":" +
  config.couchDbPort;
config.searchURL = "http://elastic:test@elasticsearch:9200";
config.webDir = __dirname + "/public";
config.serverInfo =
  "Server Information to display to users when viewing about HospitalRun";
